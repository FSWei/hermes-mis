...
Run: python3 /root/.hermes/profiles/chips/plugins/mis/test_mis.py
...
import importlib
mis = importlib.import_module("__init__") if False else None
# plugin dir isn't a package; load by path
import importlib.util
spec = importlib.util.spec_from_file_location(
    "mis_plugin", os.path.join(os.path.dirname(os.path.abspath(__file__)), "__init__.py"))
...
check("too-long flagged", len(v) == 1 and "chars" in v[0]["reason"].lower() + str(len("y"*250)), str(v))
...
check("P1 normal", mis._classify_priority("§chips-town：详见 skill chips-town") == "P2")
check("P3 temp", mis._classify_priority("临时缓存条目 Cron jobs") == "P3")
# sort order simulation: P3 first, P0 last
...
try:
    ...
    # similar body (>60 chars, >60% similar) but DIFFERENT topic → similar_entry
    ...
    # replace excludes its own old entry
...
# handler dispatch (no store needed for registry ops)
import io
